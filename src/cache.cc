...
uint32_t CACHE::cpu_redir_ipref_operate(uint64_t v_addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
    return ooo_cpu[cpu]->impl_prefetcher_cache_operate(v_addr, cache_hit, (type == PREFETCH), metadata_in);
}

uint32_t CACHE::cpu_redir_ipref_fill(uint64_t v_addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
