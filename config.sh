...
for i,cpu in enumerate(cores[:1]):
    ...
    if caches[caches[cpu['L1D']]['lower_level']]['prefetcher'] is not None:
        if os.path.exists('prefetcher/' + caches[caches[cpu['L1D']]['lower_level']]['prefetcher']):
            libfilenames['cpu' + str(i) + 'l2cprefetcher.a'] = ('prefetcher/' + caches[caches[cpu['L1D']]['lower_level']]['prefetcher'], '')
        else:
            libfilenames['cpu' + str(i) + 'l2cprefetcher.a'] = (os.path.normpath(os.path.expanduser(caches[caches[cpu['L1D']]['lower_level']]['prefetcher'])), '')
